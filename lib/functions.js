...
module.exports = class Functions {
  ...
  needed(str) {
    return bot.unicode.needed[0]+str+bot.unicode.needed[1]
  }

  optional(str) {
    return bot.unicode.optional[0]+str+bot.unicode.optional[1]
  }
  ...
  async metadataMsg(client, msg) {
    let chatMeta = async(mess) => {
      ...
      mess.isOwner = bot.ownerNumber.includes(mess.sender.jid.split('@')[0]) || mess.key.fromMe
  ...
  async run(client) {
    try {
      ...
      client.version = [2,
        2140,
        12];
      bot.session && await client.loadAuthInfo(bot.session);
